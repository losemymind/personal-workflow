...
### Example 1: React 组件审查

```text
输入: "Use code-review-skill to review this React component" (附组件代码)
流程:
  1. 阶段 1：读 PR 描述/关联 issue，确认改动范围
  2. 阶段 2：按需加载 reference/react.md —— 查 Hooks 规则、Server Components、Suspense
  3. 阶段 3：逐行检查 —— 依赖数组、闭包过期值、渲染循环、无 useMemo/useCallback 滥用
  4. 阶段 4：汇总 —— 每条发现带严重性标签
输出:
  🔴 [blocking]  useEffect 依赖缺少 `userId`，会导致陈旧数据
  🟡 [important]  loading 状态可用 useTransition 替代，避免闪烁
  🟢 [nit]       [nit] 命名 `fetchData` → `loadUser` 更贴合语义
  🌟 [praise]    错误边界与 Suspense 集成做得清晰
```

### Example 2: Java 8 遗留栈 PR 审查

```text
输入: "Review this Java 8 / Spring Boot 2 PR"
流程:
  1. 阶段 1：确认 PR 规模与测试状态
  2. 按需加载 reference/java8.md —— Stream/Optional、java.time、javax.*、RestTemplate 超时
  3. 检查 N+1（JPA / Hibernate 5）、Entity equals/hashCode、UTC 时间映射
  4. 输出分级反馈，提供可落地的修复建议
```

### Example 3: 安全审查（Go 服务）

```text
输入: "Security review of this Go service"
流程:
  1. 加载 reference/go.md + reference/security-review-guide.md
  2. 检查注入面、鉴权/授权、密钥泄漏、goroutine 生命周期、依赖漏洞
  3. 每条发现标注 🔴/🟡/🟢 分级，优先阻断高危项
```

> 全部 20+ 语言与 10+ 跨领域指南按需渐进加载；审查前先跑 `scripts/pr-analyzer.py` 对大型 diff 预判复杂度。

## Limitations and Considerations

- **不替代 linter/静态分析**：格式化、lint、简单的类型错误留给自动化工具；本技能聚焦人工审查应关注的结构、逻辑、安全、可维护性。
- **触发取决于上下文**：`description` 明确触发词（review/PR/security/architecture/performance），含糊请求可能欠触发；请显式点名本技能。
- **参考指南为通用基线**：各语言指南覆盖主流最佳实践，不保证覆盖团队特有约定/自定义框架；结合项目文档与团队规范校核。
- **脚本为辅助决策**：`pr-analyzer.py` 给出复杂度与审查路线建议，不代替人工判断与最终审批。
- **引用外部文档**：必要时 WebFetch 查阅最新文档，但对过时/不一致的依赖需以仓库实际版本为准。
- **内容来自上游**：本技能为上游 `awesome-skills/code-review-skill`（MIT）的本地化适配，保留原文与分级体系；使用与再分发须保留 LICENSE 归属。
- **不是审批系统**：输出为审查意见与行动项，合并/回滚等最终决策始终由人来拍板。
